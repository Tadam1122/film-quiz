...
io.sockets.on('connection', function (socket) {
  ...
  socket.on('newRoom', (data) => {
    ...
    data.users[0].points = 0

    //set initial room count
    data.roomCnt = 1
    data.answerCnt = 0
  ...
  socket.on('joinRoom', (data) => {
    ...
    for (let room of rooms) {
      if (room.pin === data.pin) {
        ...
        socket.join(data.pin)
        socket.emit('roomFound', { pin: room.pin })
        break
      }
    }
    if (!found) {
  ...
  socket.on('getUsernames', (data) => {
    ...
    io.sockets.to(foundPin).emit('setUsers', { usernames: usernames })
  })
      ...
      if (room.pin === data.pin) {
        for (let user of room.users) {
          if (user.id === socket.id) {
            ...
            //notify host that player submitted answer
            io.sockets
              .to(room.pin)
              .emit('answerEntered', { username: user.username })
            room.answerCnt++
            //everyone except for host submitted answers
            if (room.answerCnt === room.roomCnt - 1) {
              room.answerCnt = 0
              io.sockets
                .to(room.pin)
                .emit('playersReady', { question: data.question })
            }
          }
        }
      }
  ...
  socket.on('getAnswers', (data) => {
    for (let room of rooms) {
      if (room.pin == data.pin) {
        socket.emit('recvAnswers', { users: room.users })
      }
    }
  })
  ...
  socket.on('getHost', (data) => {
    for (let room of rooms) {
      if (room.pin == data.pin) {
        for (let user of room.users) {
          if (user.host) {
            socket.emit('sendHost', { user: user.username })
          }
        }
      }
    }
  ...
  socket.on('submitPoints', (data) => {
    ...
    io.sockets.to(data.pin).emit('getPoints', { users: foundRoom.users })
  })
